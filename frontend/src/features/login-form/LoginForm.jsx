...
function ParticleNetwork() {
  ...
  useEffect(() => {
    ...
    function initParticles() {
      const count = Math.min(70, Math.max(30, Math.floor((width * height) / 8500)))
    ...
    function draw() {
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < linkDist) {
            ctx.strokeStyle = `rgba(${LINE_RGB}, ${(1 - dist / linkDist) * 0.25})`
            ctx.lineWidth = 1
            ctx.beginPath()
            ctx.moveTo(a.x, a.y)
            ctx.lineTo(b.x, b.y)
...
export default function LoginForm() {
  ...
  const inputClass = "w-full rounded-xl border border-gray-200 bg-gray-50 pl-11 pr-4 py-3 text-sm text-slate-900 outline-none transition-all duration-200 placeholder:text-slate-400 focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100 disabled:cursor-not-allowed disabled:opacity-60"
    ...
    <div className="flex min-h-screen bg-white">
      ...
      <div className="flex items-center justify-between px-6 py-4 lg:hidden absolute top-0 left-0 right-0 z-10">
        <Link to="/" className="flex items-center gap-2 select-none">
          ...
          <span className="text-lg font-bold tracking-tight text-slate-900">Exactum</span>
        </Link>
        <Link to="/" className="flex items-center gap-1.5 text-sm font-semibold text-slate-500 hover:text-blue-600">
      ...
      <div className="relative flex w-full flex-col justify-center px-6 pt-20 pb-10 sm:px-12 lg:w-[46%] lg:pt-10 xl:w-[42%] xl:px-20">
        ...
        <div className="mb-10 hidden items-center justify-between lg:flex">
          <Link to="/" className="flex items-center gap-2 select-none">
            ...
            <span className="text-xl font-bold tracking-tight text-slate-900">Exactum</span>
          </Link>
          <Link to="/" className="flex items-center gap-1.5 text-sm font-semibold text-slate-500 transition-colors duration-200 hover:text-blue-600">
            <ArrowLeft className="h-4 w-4" />
            Voltar
          </Link>
        </div>
        ...
        <div className="fade-up mx-auto w-full max-w-sm">
          ...
          <p className="mt-2 text-sm text-slate-500">Entre com suas credenciais para acessar seu painel.</p>
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-5" noValidate>
            ...
            <div className="relative">
              <div className="mb-1.5 flex items-center justify-between ml-1">
                ...
                <span title="Disponível em breve" className="cursor-not-allowed text-xs font-semibold text-slate-300">
                  Esqueceu a senha?
                </span>
              </div>
              <div className="relative">
          ...
          <p className="mt-6 text-center text-sm text-slate-500">
            ...
            <Link to="/create-tenant" className="font-semibold text-blue-600 hover:text-blue-700">
              Criar agora
            </Link>
          </p>
        </div>
      ...
      <div className="relative hidden overflow-hidden bg-gradient-to-br from-blue-700 via-blue-800 to-slate-900 lg:flex lg:w-[54%] xl:w-[58%]">
        ...
        <div
          ...
          style={{
            background: "radial-gradient(ellipse 80% 80% at 30% 30%, rgba(29,78,216,0.35), transparent 60%)",
          }}
        />
        ...
        <div className="relative z-10 flex w-full flex-col justify-between p-12 xl:p-16">
          ...
          <div className="max-w-lg">
            ...
            <div className="mt-8 h-[72px]">
              <p key={phraseIndex} className="fade-in text-lg leading-relaxed text-blue-100">
              ...
              {VALUE_PROPS.map((_, i) => (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all duration-300 ${
                    i === phraseIndex ? "w-6 bg-white" : "w-1.5 bg-white/30 hover:bg-white/50"
                  }`}
                />
              ))}
            </div>
            ...
            <div className="mt-12 flex flex-col gap-4">
              <div className="float-slow flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3 backdrop-blur-sm">
                ...
                <span className="text-sm font-medium text-blue-50">Gestão centralizada da operação</span>
              </div>
              <div className="float-slow flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3 backdrop-blur-sm" style={{ animationDelay: "1.2s" }}>
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/10 text-blue-200">
                  <Bell className="h-4 w-4" />
                </div>
                <span className="text-sm font-medium text-blue-50">Alertas inteligentes de estoque</span>
              </div>
              <div className="float-slow flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3 backdrop-blur-sm" style={{ animationDelay: "2.4s" }}>
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/10 text-blue-200">
                  <Sparkles className="h-4 w-4" />
                </div>
                <span className="text-sm font-medium text-blue-50">
                  Previsão de demanda com IA <span className="text-blue-300"> (em breve)</span>
                </span>
              </div>
            </div>
          </div>

          <p className="fade-up text-xs text-blue-200/70" style={{ animationDelay: "160ms" }}>
            Exactum Tecnologia · Construído para empresas que querem crescer com dados.
          </p>
        </div>
      </div>
    </div>
