...
function StatCard({ icon: Icon, label, value, unit, tone = "blue" }) {
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-[0_1px_2px_rgba(15,23,42,0.02),0_8px_20px_-14px_rgba(15,23,42,0.1)]">
      ...
      <h3 className="text-2xl font-black tracking-tight text-slate-900" style={{ fontVariantNumeric: "tabular-nums" }}>
        {value}
  ...
  if (!sale && !loading) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-3 bg-gray-50 p-8 text-center">
        ...
        <p className="text-sm font-medium text-slate-600">Venda não encontrada ou erro ao carregar.</p>
        <button onClick={() => navigate(-1)} className="text-sm font-bold text-blue-600 hover:text-blue-700 hover:underline">
          Voltar para a lista
        </button>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-slate-900">Detalhes da Venda</h1>
            <p className="text-sm font-medium text-slate-500">
              Pedido #{sale?.uuid ?? sale?.id} • {sale?.created_at ? new Date(sale?.created_at).toLocaleDateString('pt-BR') : 'Data Indisponível'}
            </p>
          </div>
        </div>
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-[0_1px_2px_rgba(15,23,42,0.02),0_8px_20px_-14px_rgba(15,23,42,0.1)]">
          <div className="mb-3 flex items-center gap-3">
            ...
            <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Informações Gerais</p>
          </div>
          <p className="text-sm font-semibold text-slate-600">
            Pagamento: <span className="text-slate-800">{sale?.payment_method || "N/A"}</span>
          </p>
          <p className="text-sm font-semibold text-slate-600">
            Hora:{" "}
        ...
        <StatCard icon={ShoppingCart} label="Volume de Itens" value={totalUnits} unit="unidades" tone="purple" />
        <StatCard icon={Tag} label="Valor Total" value={formatCurrency(sale?.total_price)} tone="emerald" />
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.02),0_8px_20px_-14px_rgba(15,23,42,0.1)]">
        ...
        <div className="max-h-[400px] overflow-x-auto overflow-y-auto">
          <table className="w-full border-collapse">
            <thead className="sticky top-0 z-10 bg-gray-50 shadow-sm">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-bold uppercase text-slate-500">SKU</th>
                <th className="px-6 py-4 text-left text-xs font-bold uppercase text-slate-500">Produto</th>
                <th className="px-6 py-4 text-center text-xs font-bold uppercase text-slate-500">Qtd</th>
                <th className="px-6 py-4 text-right text-xs font-bold uppercase text-slate-500">Subtotal</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {items.map((item) => (        
                <tr key={item.id} className="transition-colors duration-200 hover:bg-gray-50/80">
                  ...
                  <td className="px-6 py-4 text-right text-sm font-black text-slate-800" style={{ fontVariantNumeric: "tabular-nums" }}>
                    {formatCurrency(Number(item.item_price || 0) * item.quantity)}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="border-t border-gray-100 bg-gray-50/50">
          <div className="flex items-center justify-end px-6 py-4">
             <span className="mr-8 flex items-center text-sm font-bold uppercase tracking-widest text-slate-500">Total do Pedido:</span>
             <span className="text-xl font-black text-blue-600" style={{ fontVariantNumeric: "tabular-nums" }}>
