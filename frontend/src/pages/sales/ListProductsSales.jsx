...
export default function ListSoldItems() {
  ...
  const filteredSoldItems = soldItems.filter(
    (item) =>
  ...
  const totalItemsSold = soldItems.reduce(
    (acc, item) => acc + parseFloat(item.total_quantity),
    0
  );
      ...
      <div className="animate-in fade-in duration-500 pb-10 h-full min-h-0 overflow-y-auto pr-3 custom-scroll">
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm font-medium text-slate-500">
              ...
              {monthOptions.find((m) => m.value === month)?.label} de {year}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 mb-6">
          <StatCard
            ...
            value={`${filteredSoldItems.length} ${
              filteredSoldItems.length === 1 ? "produto" : "produtos"
            }`}
            tone="blue"
          />
          <StatCard
            ...
            value={`${totalItemsSold} ${
              totalItemsSold === 1 ? "unidade" : "unidades"
            }`}
            tone="emerald"
          />
          <StatCard
          ...
          <div className="mt-6 flex flex-col items-center justify-between gap-3 sm:flex-row">
            <p className="text-xs text-slate-500">
              ...
              <span className="font-semibold text-slate-700">
                {startIndex + 1}–{Math.min(endIndex, filteredSoldItems.length)}
              </span>{" "}
              de{" "}
              <span className="font-semibold text-slate-700">
