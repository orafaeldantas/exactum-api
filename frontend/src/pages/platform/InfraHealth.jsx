...
function StatusPill({ status }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${styles}`}
    >
...
export default function InfraHealth() {
  ...
  const services = infraHealth?.services
    ? Object.entries(infraHealth.services)
    : [];
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-10 h-full min-h-0 overflow-y-auto pr-3 custom-scroll">
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Status dos serviços e dependências do ecossistema
          </p>
        </div>
        <button
          onClick={loadHealth}
      ...
      <div
        className={`mb-6 flex flex-col gap-4 rounded-2xl border p-5 shadow-[0_1px_2px_rgba(15,23,42,0.02),0_8px_20px_-14px_rgba(15,23,42,0.1)] sm:flex-row sm:items-center sm:justify-between ${
          isHealthy
      ...
      >
        ...
        {infraHealth?.timestamp && (
          <span className="flex items-center gap-1.5 text-xs text-slate-400">
            ...
            Verificado às{" "}
            {new Date(infraHealth.timestamp).toLocaleTimeString("pt-BR")}
          </span>
        )}
      </div>
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <InfoCard
          icon={Tag}
          label="Versão"
          value={infraHealth?.version ?? "—"}
          tone="blue"
        />
        <InfoCard
          icon={Timer}
          label="Uptime"
          value={infraHealth ? formatUptime(infraHealth.uptimeSeconds) : "—"}
          tone="slate"
        />
        <InfoCard
          icon={AlertTriangle}
        ...
        {services.map(([name, service]) => {
          ...
          return (
            ...
            >
              ...
              <h3 className="text-lg font-bold capitalize leading-tight text-slate-800">
                {name}
              </h3>
