...
const VendaLocal = () => {
  ...
  // Filter products based on search input (uuid or Name)
    ...
    if (value.length > 0) {
      const filtered = productsDb.filter(
        (p) =>
          ...
          p.uuid.toString().includes(value)
      );
  ...
  const addToCart = (product) => {
    const existing = cartItems.find((item) => item.uuid === product.uuid);
    if (existing) {
      setCartItems(
        cartItems.map((item) =>
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`w-2 h-2 rounded-full animate-pulse ${
                    isDbConnected ? "bg-emerald-500" : "bg-red-500"
                  }`}
                ></span>
                <span
                  className={`text-xs font-bold ${
                    isDbConnected ? "text-emerald-600" : "text-red-600"
                  }`}
                >
            ...
            <div className="absolute top-full left-0 w-full bg-white mt-2 rounded-xl shadow-2xl border border-slate-200 max-h-72 overflow-y-auto z-50">
              {searchSuggestions.map((p) => (
                <div
                  key={p.uuid}
                  onClick={() => addToCart(p)}
                  className="p-4 hover:bg-blue-50 cursor-pointer flex justify-between items-center border-b border-slate-50 last:border-0 group"
                >
                    ...
                    <span
                      className={
                        p.stock_quantity > 0
                          ...
                          : "text-[10px] text-red-400 font-mono bg-red"
                      }
                    >
                      UUID: {p.uuid} |{" "}
                      {p.stock_quantity > 0
          ...
          <div className="overflow-y-auto flex-1">
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50 sticky top-0 z-10 border-b border-slate-200">
                <tr>
                  <th className="p-4 font-semibold text-slate-600 text-[10px] uppercase">
                    Produto
                  </th>
                  <th className="p-4 font-semibold text-slate-600 text-[10px] uppercase text-center">
                    Qtd
                  </th>
                  <th className="p-4 font-semibold text-slate-600 text-[10px] uppercase">
                    Unitário
                  </th>
                  <th className="p-4 font-semibold text-slate-600 text-[10px] uppercase text-right">
                    Subtotal
                  </th>
                  <th className="p-4"></th>
                </tr>
                ...
                {cartItems.map((item) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-right font-bold text-slate-900 text-sm">
                      R$ {(item.itemPrice * item.quantity).toFixed(2)}
                    </td>
      ...
      <aside className="w-96 flex flex-col gap-4">
        ...
        <div className="bg-slate-900 text-white p-6 rounded-3xl shadow-xl border border-slate-800">
          <div className="flex justify-between items-center mb-4">
            <button
              className="flex items-center gap-2 text-slate-400 hover:text-white text-[10px] font-black uppercase tracking-widest group"
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex-1 overflow-hidden flex flex-col">
          <h3 className="font-black text-[10px] uppercase tracking-widest text-slate-400 flex items-center gap-2 mb-4">
            <AlertTriangle size={14} className="text-blue-600" /> Exactum AI
            Insights
          </h3>
            ...
            {cartItems.length > 0 ? (
              <div className="p-4 rounded-2xl bg-amber-50 border-l-4 border-amber-400 animate-in slide-in-from-right">
                ...
                <p className="text-xs text-amber-800 leading-tight">
                  <b>{cartItems[cartItems.length - 1]?.name}</b> atingirá o
                  estoque zero em{" "}
                  <b>{cartItems[cartItems.length - 1]?.prediction} dias</b>.
                </p>
              </div>
            ) : (
              <p className="text-[11px] text-slate-400 italic text-center mt-10 px-6 font-medium">
                Bipe um produto para análise preditiva de estoque em tempo real.
              </p>
            )}
          </div>
        </div>
        ...
        <div className="bg-slate-900 text-white p-6 rounded-3xl shadow-2xl border border-slate-800 space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-5 gap-1.5">
              {[
                ...
                >
                  <m.icon size={16} />
                  <span className="text-[8px] font-bold mt-1 uppercase leading-none">
                    {m.label}
                  </span>
                </button>
              ))}
              ...
              >
                {[...Array(12)].map((_, i) => (
                  <option key={i + 1} value={i + 1}>
                    {i + 1}x de R$ {(totalToPay / (i + 1)).toFixed(2)}
                  </option>
                ))}
              </select>
            )}
            ...
            <div className="flex items-center justify-between bg-slate-800/50 p-3 rounded-xl border border-slate-700">
              ...
              <div className="flex items-center gap-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setAmountReceived(e.target.value.replace(",", "."))
                  }
          ...
          <div className="space-y-3 pt-2">
            <div className="flex items-center justify-between bg-slate-800/50 p-3 rounded-xl border border-slate-700">
              ...
              <input
                ...
                onChange={(e) =>
                  setDiscountPercent(Number(e.target.value.replace(/\D/g, "")))
                }
                onFocus={(e) => e.target.select()}
              />
            </div>
          ...
          <button
            className="group w-full bg-blue-600 hover:bg-blue-500 py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] 
