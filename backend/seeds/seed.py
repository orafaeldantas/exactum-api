...
def generate_sales(
    tenant_id: int, user_id: int, start_year: int, end_year: int, max_month: int = 12
):
    ...
    for year in range(start_year, end_year + 1):
        ...
        for month in range(1, months_limit + 1):
            success = sales_database_seed(
                tenant_id=tenant_id, user_id=user_id, month=month, year=year
            )
            if not success:
                raise RuntimeError(
...
if __name__ == "__main__":
    ...
    with app.app_context():
        ...
        products = [product_one, product_two, product_three, product_four, product_five]
        tenants_created = []
        ...
        for tenant, product in zip(tenants, products):
            ...
            if not products_database_seed(tenant_id, product):
                raise RuntimeError(f"Error seeding products for the tenant: {tenant}")
